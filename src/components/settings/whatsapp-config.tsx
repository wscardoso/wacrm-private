...
import { buildWebhookUrl } from '@/lib/whatsapp/webhook-url';
import {
  Accordion,
  AccordionItem,
  AccordionTrigger,
...
export function WhatsAppConfig() {
  ...
  // Non-Meta webhook identity — set by the server (POST /api/whatsapp/config),
  // never derived from the user-typed Verify Token field. connection_id is
  // safe to persist/reload; the plaintext webhook secret is generated by
  // bootstrapConnection() and revealed exactly once, in the save response —
  // it is never stored, so it cannot be recovered on a later page load.
  const [connectionId, setConnectionId] = useState('');
  const [revealedWebhookSecret, setRevealedWebhookSecret] = useState<string | null>(null);

  // Webhook URL — contract: /api/whatsapp/webhook/{provider}/{connectionId}/{webhookSecret}
  const origin = typeof window !== 'undefined' ? window.location.origin : '';
  const webhookUrl = buildWebhookUrl({ origin, provider, connectionId, webhookSecret: revealedWebhookSecret });
  ...
  const fetchConfig = useCallback(async (acctId: string) => {
    ...
    try {
      ...
      // connection_id is safe to reload from the DB row; the plaintext
      // webhook secret never is (it's only ever known right after the
      // save call that generated it).
  ...
  async function handleSave() {
    ...
    try {
      ...
      // Non-Meta: capture the bootstrapped connection identity. The secret
      // is only ever present in THIS response — right after the server
      // generated it — never on a later reload.
      if (provider !== 'meta') {
        if (data.connection_id) setConnectionId(data.connection_id);
        setRevealedWebhookSecret(data.webhook_secret ?? null);
      }
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              {/* Webhook Verify Token (Meta only — legacy field for non-Meta, unused since 036) */}
              <div className="space-y-2">
                ...
                {provider === 'meta' ? (
                  ...
                  <p className="text-xs text-muted-foreground">
                    Campo legado, preservado por compatibilidade — não é mais usado para autenticar o webhook.
                    A URL do webhook abaixo já inclui um segredo gerado automaticamente pelo servidor.
                  </p>
                )}
              </div>
              ...
              <div className="space-y-2">
                ...
                {provider !== 'meta' && !connectionId && (
                  <p className="text-xs text-amber-400">
                    Salve a configuração para gerar a URL final do webhook.
                  </p>
                )}
                {provider !== 'meta' && connectionId && revealedWebhookSecret && (
                  <p className="text-xs text-amber-400">
                    Copie esta URL agora — o segredo não será exibido novamente depois que você sair desta página.
                  </p>
                )}
                {provider !== 'meta' && connectionId && !revealedWebhookSecret && (
                  <p className="text-xs text-muted-foreground">
                    Webhook já configurado anteriormente. Por segurança, o segredo não é exibido novamente —
                    use a URL que você já configurou no painel do {provider === 'zapi' ? 'Z-API' : 'uazapi'}.
                    Se ela foi perdida, contate o suporte para rotacionar o segredo.
                  </p>
                )}
              </div>
            </CardContent>
