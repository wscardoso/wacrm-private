...
**Versão:** v1.6 · **Snapshot auditado:** HEAD `26e5d39` (fechamentos documentais de 2026-07-29/30 aplicados sobre este snapshot; nenhuma alteração de código considerada — v1.6 registra hardening operacional posterior, ver histórico, sem reauditar épicos)
...
- **v1.6** (2026-09-10) — **nota operacional, mesmo padrão da v1.3 (Sprint C): hardening de segurança pós-auditoria E2E + fechamento do V0 de deploy.** Não altera épicos E0–E13, a cadeia de precedência, nem o snapshot auditado (`26e5d39`) — trabalho de segurança/infra, não escopo restante de messaging.
  - **Backlog residual da auditoria E2E fechado (4 itens):** `F-DB-03` — CLOSED, INTENTIONAL: ausência de DML `authenticated` em `flow_runs`/`automation_logs` confirmada como modelo de acesso deliberado (todo write é service-role ou `SECURITY DEFINER`), sem alteração de código. `F-INT-06` — CLOSED, CONFIRMED-DRIFT: comentários do orphan-sweep/retry-policy corrigidos de `messages.created_at` para o `outbound_retry_ledger.created_at` real (commit `58e7e2d`). `F-ENV-02` — CLOSED, HARDENING: `/act` adicionado a `protectedPaths` do proxy — defense-in-depth, sem bypass ativo encontrado (commit `7ca4463`). `F-DB-04` — CORRECTED: migration `079` revoga o privilégio de tabela INSERT/UPDATE/DELETE não utilizado que `authenticated` ainda herdava do bootstrap `GRANT ALL` do Supabase em `flow_runs`/`automation_logs` — mesma classe de defense-in-depth de `074`/`078` (commit `943077a`).
  - **V0 (agendamento real do orphan-sweep/TTL sweep — antes DEFERRED) fechado.** Deploy migrou de Hostinger para VPS própria via Coolify. No caminho: lockfile corrigido (`npm ci` quebrado no build, commit `0bb7fe6`); `AUTOMATION_CRON_SECRET` nunca havia sido configurada em produção — criada (Runtime apenas, sem Buildtime, nos 4 secrets reais do projeto). **Achado de bug real, não coberto pela auditoria original:** o gate de sessão de usuário em `src/proxy.ts` para `/api/whatsapp/*` bloqueava incondicionalmente `delivery/cron`, `orphan-sweep` e `kid-convergence-sweep` mesmo com `x-cron-secret` válido — esses 3 endpoints nunca foram alcançáveis por um pinger externo desde que essa regra existe. Corrigido com a mesma exceção já concedida a `/webhook` (commit `860bf2f`, testes de regressão em `proxy.test.ts`). Os 6 endpoints de cron/sweep confirmados funcionais ponta a ponta e agendados via Coolify Scheduled Tasks (cadência por prioridade de negócio: 2–5 min para os críticos de fila, 30 min para `flows/cron`, 6h para `kid-convergence-sweep`).
  - **Achado registrado, não corrigido nesta sessão:** `npm audit` acusa 1 vulnerabilidade crítica e 1 alta em `next`/`sharp` (pré-existente, `next@16.2.6` inalterado) — correção sugerida sai do range declarado em `package.json` e exige revisão própria.
  - Commits (`main`): `58e7e2d`, `7ca4463`, `943077a`, `0bb7fe6`, `860bf2f`.
